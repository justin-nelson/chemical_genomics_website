<?php
	...
	*/
	$aColumns = array(	'c.chemical_name', 
				'g.GO_name', 
				'g.GO_desc', 
				'TRUNCATE(p.worst_pval,4)', 
				'TRUNCATE(p.worst_zscore,4)', 
				'TRUNCATE(p.dmso_FDR,4)', 
				'TRUNCATE(p.rand_by_strain_FDR,4)', 
	...
	/* DB table to use */
	$sTable = $GLOBALS["TABLE_PROCESS_TARGET_PREDICTION"]." as p, ".$GLOBALS["TABLE_CHEMICAL_NAMES"]." as c, ".$GLOBALS["TABLE_GO_NAMES"]." as g";
	...
	*/
	...
	if ( $sWhere == "" ){ $sWhere = "WHERE "; } else { $sWhere .= " AND "; }
	$sWhere .= " p.worst_pval < 0.05 AND p.chemid = c.pid AND p.GOid = g.pid AND ";

	if($_GET['PID'] != ""){
		if($_GET['searchType'] == "CHEMICAL") {
			$sWhere .= "p.chemid = '".mysql_real_escape_string($_GET['PID'])."'";
     		} elseif($_GET['searchType'] == "GO") {
			$sWhere .= "p.GOid = '".mysql_real_escape_string($_GET['PID'])."'";
		} else {
			$sWhere .= "1 = 0";
		}
