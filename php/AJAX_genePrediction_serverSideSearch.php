<?php
    ...
    /* DB table to use */
    $sTable = $GLOBALS["TABLE_GENE_TARGET_PREDICTION"]." as g, ".$GLOBALS["TABLE_CHEMICAL_NAMES"]." as c, ".$GLOBALS["TABLE_GENE_NAMES"]." as o";
	...
	*/
	...
	if ( $sWhere == "" ){ $sWhere = "WHERE "; } else { $sWhere .= " AND "; }
	$sWhere .= " g.chemid = c.pid AND g.geneid = o.pid AND ";

	if($_GET['PID'] != ""){
		if($_GET['searchType'] == "CHEMICAL"){
			$sWhere .= "g.chemid = '".mysql_real_escape_string($_GET['PID'])."'";
     		} elseif($_GET['searchType'] == "GENE"){
			$sWhere .= "g.geneid = '".mysql_real_escape_string($_GET['PID'])."'";
		} else {
			$sWhere .= "1 = 0";
		}
