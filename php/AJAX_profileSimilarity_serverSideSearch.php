<?php
     ...
     */
    $aColumns = array( 'chemical1_name', 'chemical2_name', 'TRUNCATE(score,4)', 'IF(chemid2 IN (SELECT DISTINCT chemid FROM '.$GLOBALS["TABLE_PROCESS_TARGET_PREDICTION"].' WHERE GREATEST(dmso_FDR, rand_by_strain_FDR) < 0.25),TRUE,FALSE)');
     
    /* Indexed column (used for fast and accurate table cardinality) */
    $sIndexColumn = "pid";
     
    /* DB table to use */
    $sTable = $GLOBALS["TABLE_PROFILE_SIMILARITY"];
	...
	if($_GET['PID'] != ""){
		if($_GET['searchType'] == "CHEMICAL"){
			$sWhere .= "chemid1 = '".mysql_real_escape_string($_GET['PID'])."'";
		} else {
			$sWhere .= "1 = 0";
		}
